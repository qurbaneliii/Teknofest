...
AUDITED_DECISION_PATH = METRICS_DIR / "audited_final_model_decision.json"
LEGACY_DECISION_PATH = METRICS_DIR / "final_model_decision.json"


def default_decision_path() -> Path:
    """Prefer the independently audited decision when it is available."""
    return AUDITED_DECISION_PATH if AUDITED_DECISION_PATH.exists() else LEGACY_DECISION_PATH


def load_final_decision(path: str | Path | None = None) -> dict[str, Any]:
    decision_path = Path(path) if path is not None else default_decision_path()
...
def generate_final_submission(
    ...
    if basic_submission:
        output = output[["Variant_ID", "predicted_label"]]
...
def main() -> None:
    """CLI entrypoint for label-free organizer-format inference."""
    import argparse

    parser = argparse.ArgumentParser(description="Generate TEKNOFEST predictions without using input labels.")
    parser.add_argument("--input", required=True, type=Path, help="Organizer-format CSV; Label is optional and ignored.")
    parser.add_argument("--output", required=True, type=Path, help="Destination CSV.")
    parser.add_argument("--decision", type=Path, default=None, help="Decision JSON (defaults to audited artifact when present).")
    parser.add_argument("--basic-submission", action="store_true", help="Write only Variant_ID and predicted_label after confirming the organizer template.")
    args = parser.parse_args()
    output, destination = generate_final_submission(
        pd.read_csv(args.input), args.output, args.decision, basic_submission=args.basic_submission
    )
    print(f"Submission written to: {destination.resolve()} ({len(output)} rows)")
